...
for (const url of urls) {
  ...
  console.log(`level=${m.level} version=${m.version} size=${m.size} carved=${m.carvedCount} ` +
    `damage=${(r.attempts.at(-1)!.damageRatio * 100).toFixed(2)}%`);
  console.log(`start=(${m.start.row},${m.start.col}) end=(${m.end.row},${m.end.col}) ` +
    `moves=${m.analysis.shortestLength} routes=${m.analysis.shortestRouteCount} ` +
    `reachable=${m.analysis.reachableCells}`);
